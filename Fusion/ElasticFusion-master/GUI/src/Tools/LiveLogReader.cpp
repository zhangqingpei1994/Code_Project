...
LiveLogReader::LiveLogReader(std::string file, bool flipColors, CameraType type)
 : LogReader(file, flipColors),       //LogReader(file, flipColors)对基类进行初始化
   lastFrameTime(-1),
   lastGot(-1)
{
    ...
    decompressionBufferDepth = new Bytef[Resolution::getInstance().numPixels() * 2];  //定义numPixels() * 2大小的uchar类型的内存
    ...
    else
    {
        ...
        do
        {
          std::this_thread::sleep_for(std::chrono::microseconds(33333));
            std::cout << "."; std::cout.flush();  //刷新输出缓冲器并输出
...
void LiveLogReader::getNext()
{
    ...
    imageSize = Resolution::getInstance().numPixels() * 3;    //RGB各占一个字节
    depthSize = Resolution::getInstance().numPixels() * 2;    //short是两个字节长度
