    ...
    else
    {
        ...
        else
        {
            ...
            if(initSuccessful)
            {
                ...
                setAutoExposure(true);       //自动曝光
                setAutoWhiteBalance(true);   //自动白平衡

                rgbStream.addNewFrameListener(rgbCallback);           //中断方式获取相机数据
                depthStream.addNewFrameListener(depthCallback);       //中断方式获取相机数据
            }
        }
    }
}
