...
namespace kfusion
{
    namespace device
    {
     __global__ void bilateral_kernel(const PtrStepSz<float> src, PtrStep<float> dst, const int ksz, const float sigma_spatial2_inv_half, const float sigma_depth2_inv_half)
...
namespace kfusion
{
    namespace device
    {
       // __global__ void truncate_depth_kernel(PtrStepSz<ushort> depth, ushort max_dist /*mm*/)
        __global__ void truncate_depth_kernel(PtrStepSz<float> depth, ushort max_dist /*mm*/)
...
namespace kfusion
{
    namespace device
    {
      ...
      __global__ void pyramid_kernel(const PtrStepSz<float> src, PtrStepSz<float> dst, float sigma_depth_mult3)
        {
            int x = blockIdx.x * blockDim.x + threadIdx.x;
            int y = blockIdx.y * blockDim.y + threadIdx.y;
...
namespace kfusion
{
    namespace device
    {
       // __global__ void compute_normals_kernel(const PtrStepSz<ushort> depth, const Reprojector reproj, PtrStep<Normal> normals)
        __global__ void compute_normals_kernel(const PtrStepSz<float> depth, const Reprojector reproj, PtrStep<Normal> normals)
       ...
       // __global__ void mask_depth_kernel(const PtrStep<Normal> normals, PtrStepSz<ushort> depth)
        __global__ void mask_depth_kernel(const PtrStep<Normal> normals, PtrStepSz<float> depth)
...
namespace kfusion
{
    namespace device
    {
       // __global__ void points_normals_kernel(const Reprojector reproj, const PtrStepSz<ushort> depth, PtrStep<Point> points, PtrStep<Normal> normals)
        __global__ void points_normals_kernel(const Reprojector reproj, const PtrStepSz<float> depth, PtrStep<Point> points, PtrStep<Normal> normals)
...
namespace kfusion
{
    namespace device
    {
        ...
        __global__ void compute_dists_kernel(const PtrStepSz<float> depth, Dists dists, float2 finv, float2 c)
...
namespace kfusion
{
    namespace device
    {
       // __global__ void resize_depth_normals_kernel(const PtrStep<ushort> dsrc, const PtrStep<float4> nsrc, PtrStepSz<ushort> ddst, PtrStep<float4> ndst)
        __global__ void resize_depth_normals_kernel(const PtrStep<float> dsrc, const PtrStep<float4> nsrc, PtrStepSz<float> ddst, PtrStep<float4> ndst)
...
namespace kfusion
{
    namespace device
    {
        ...
        __global__ void render_image_kernel(const PtrStep<float> depth, const PtrStep<Normal> normals,
                                           const Reprojector reproj, const float3 light_pose, PtrStepSz<uchar4> dst)
        {
            int x = threadIdx.x + blockIdx.x * blockDim.x;
            int y = threadIdx.y + blockIdx.y * blockDim.y;
...
void kfusion::device::renderImage(const Depth& depth, const Normals& normals, const Reprojector& reproj, const float3& light_pose, Image& image)
{
    ...
    render_image_kernel<<<grid, block>>>((PtrStep<float>)depth, normals, reproj, light_pose, image);
