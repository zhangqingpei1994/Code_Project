...
kfusion::KinFuParams kfusion::KinFuParams::default_params()
{
    ...
    p.cols = 512;  //pixels 640
    p.rows = 424;  //pixels 480
...
bool kfusion::KinFu::operator()(const kfusion::cuda::Depth& depth, const kfusion::cuda::Image& /*image*/)
{
    ...
    //cout<<p.intr.fx<<" "<<p.intr.fy<<endl;

    cuda::computeDists(depth, dists_, p.intr);       //计算距离，不是深度--

    //深度图像双边滤波,滤波后的图像存在curr_.depth_pyr[0]中
