...
{
    float2 coo;
    coo.x = __fmaf_rn(f.x, __fdividef(p.x, p.z), c.x);  //(f.x*p.x/p.z)+c.x
...
__kf_device__ uchar3 kfusion::device::pack_tsdf (uchar R, uchar G, uchar B)
{
...
//__kf_device__ float kfusion::device::unpack_tsdf (uchar3 value)
//{
//    return __half2float (value.x);
//}
