...
const CudaData kfusion::cuda::TsdfVolume::data() const
{
    return data_;
}
...
{ 
    device::Vec3i dims = device_cast<device::Vec3i>(dims_);  //device::Vec3i:  int3
    device::Vec3f vsz  = device_cast<device::Vec3f>(getVoxelSize());  // device::Vec3f: float3

    device::TsdfVolume volume(data_.ptr<ushort2>(), dims, vsz, trunc_dist_, max_weight_);

    //std::cout<<volume.dims.x<<" "<<volume.dims.y<<" "<<volume.dims.z<<std::endl;
...
{
    Affine3f vol2cam = camera_pose.inv() * pose_;  //Affine3f是一个 4*4的仿射变换矩阵

    device::Projector proj(intr.fx, intr.fy, intr.cx, intr.cy);

    device::Vec3i dims = device_cast<device::Vec3i>(dims_);  //device::Vec3i:  int3
    device::Vec3f vsz  = device_cast<device::Vec3f>(getVoxelSize());    //Vec3f ： float3,   getVoxelSize()返回的是cv::Vec3f,3通道的float
...
DeviceArray<Point> kfusion::cuda::TsdfVolume::fetchCloud(DeviceArray<Point>& cloud_buffer) const
{
    ...
    size_t size = extractCloud(volume, aff, b);                 //在internal.hpp中有定义
