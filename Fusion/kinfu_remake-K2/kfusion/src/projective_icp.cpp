...
struct kfusion::cuda::ProjectiveICP::StreamHelper
{
    ...
    typedef cv::Matx66f Mat6f;  //6*6
    typedef cv::Vec6f Vec6f;    //就是6*1的一个vector
...
{
    int i = MAX_PYRAMID_LEVELS - 1;     //最大金字塔等级 MAX_PYRAMID_LEVELS=4
    for(; i >= 0 && !iters_[i]; --i);
    return i + 1;
}
...
bool kfusion::cuda::ProjectiveICP::estimateTransform(Affine3f& affine, const Intr& intr, const DepthPyr& dcurr, const NormalsPyr ncurr, const DepthPyr dprev, const NormalsPyr nprev)
{
    ...
    for(int level_index = LEVELS - 1; level_index >= 0; --level_index)
    {
        ...
        helper.setLevelIntr(level_index, intr.fx, intr.fy, intr.cx, intr.cy);  //设置当前层的相机内参 --
...
{
    const int LEVELS = getUsedLevelsNum();  //LEVELS=3 一直 --
    StreamHelper& sh = *shelp_;

    device::ComputeIcpHelper helper(dist_thres_, angle_thres_); // dist_thres_=0.1  angle_thres_=20度
    affine = Affine3f::Identity();
    ...
    for(int level_index = LEVELS - 1; level_index >= 0; --level_index)
    {
        ...
        for(int iter = 0; iter < iters_[level_index]; ++iter)
        {
            ...
            if (fabs (det) < 1e-15 || cv::viz::isNan (det))   //isNan检查是不是数字 --
            {
                if (cv::viz::isNan (det)) cout << "qnan" << endl;
                return false;
            }
            ...
            StreamHelper::Vec6f r;
            cv::solve(A, b, r, cv::DECOMP_SVD);  //求解方程
