...
namespace kfusion
{
    namespace device
    {
        ...
        int ComputeIcpHelper::find_coresp(int x, int y, float3& nd, float3& d, float3& s) const
        {
            s = tr(vcurr(y, x));  //当前帧的三维点云信息  --
            if (isnan(s.x))
                return 40;

            s = aff * s;          // 转换到上一帧的三维点 --

            float2 coo = proj(s); //对应的像素坐标
            if (s.z <= 0 || coo.x < 0 || coo.y < 0 || coo.x >= cols || coo.y >= rows)
                return 80;

            d = tr(tex2D(vprev_tex, coo.x, coo.y)); //找到对应像素坐标在上一帧中的点云信息
            if (isnan(d.x))
                return 120;
...
///////////////////////////////////////////////////
        ...
        __global__ void icp_helper_kernel(const ComputeIcpHelper helper, PtrStep<float> partial_buf)
        {
            ...
            float3 n, d, s; //d：上一帧对应点三维点云坐标，s：当前帧映射过去的点云坐标，n：上一帧的法线信息。
...
void kfusion::device::ComputeIcpHelper::operator()(const Points& vprev, const Normals& nprev, DeviceArray2D<float>& buffer, float* data, cudaStream_t s)
{
    ...
    cudaSafeCall ( cudaMemcpyAsync(data, buffer.ptr(Policy::TOTAL), Policy::TOTAL * sizeof(float), cudaMemcpyDeviceToHost, s) );  //把数据保存到主机上
    cudaSafeCall ( cudaGetLastError () );
}
...
{ cudaSafeCall( cudaMallocHost((void **)&data, Policy::TOTAL * sizeof(float)) );  }   //cudaMallocHost:页锁定内存
