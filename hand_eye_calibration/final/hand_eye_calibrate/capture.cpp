...
#define REPORT_ERROR(msg) error ((msg), __FILE__, __LINE__)*/


struct OpenNISource::Impl
{
    Context context;
...
OpenNISource::OpenNISource() : depth_focal_length_VGA (0.f), baseline (0.f),
    shadow_value (0), no_sample_value (0), pixelSize (0.0), max_depth (0) {}

OpenNISource::OpenNISource(int device) {open (device); }
OpenNISource::OpenNISource(const string& filename, bool repeat /*= false*/) {open (filename, repeat); }
OpenNISource::~OpenNISource() { release (); }

void OpenNISource::open (int device)
{
...
void OpenNISource::open(const std::string& filename, bool repeat /*= false*/)
{
    impl_ = cv::Ptr<Impl> ( new Impl () );
...
bool OpenNISource::grab(cv::Mat& depth, cv::Mat& image)
{
...
bool OpenNISource::setRegistration (bool value)
{
    XnStatus rc = XN_STATUS_OK;
