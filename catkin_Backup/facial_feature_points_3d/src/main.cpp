...
int main(int argc, char **argv)
{
  ...
  std::thread pointcloud_viewer_thread;    //点云显示和人脸特征点定位线程

  std::string ns = K2_DEFAULT_NS;   //"kinect2"
  std::string topicColor = K2_TOPIC_SD K2_TOPIC_IMAGE_COLOR K2_TOPIC_IMAGE_RECT;    //这种赋值不太理解，把HD改成QHD可以变分辨率，两个都改一下
  ...
  points_3d_show.Inital();

  pointcloud_viewer_thread = std::thread(&Points_3d_show::cloudViewer, &points_3d_show);

  while(ros::ok());

  receiver.stop();

  pointcloud_viewer_thread.join();
