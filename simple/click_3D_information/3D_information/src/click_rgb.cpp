...
private:
  ...
  for(; running && ros::ok();)
  {
    if(updateCloud)
    {
     ...
     /* ++frameCount;
      ...
      cv::putText(color, ossXYZ.str(), cv::Point(img_x_click+10, img_y_click), font, 0.6, colorText, 3, CV_AA);

      cv::circle(color, cv::Point(img_x_click, img_y_click), 2, cv::Scalar(0, 255, 0), -1);
      ...
      cv::putText(color, ossXYZ.str(), cv::Point(img_x_click_last+10, img_y_click_last), font, 0.6, colorText, 3, CV_AA);

      cv::circle(color, cv::Point(img_x_click_last, img_y_click_last), 2, cv::Scalar(0, 255, 0), -1);
      //============================================================================================================

      std::cout<< "( " << ptMsg_temp.point.x << ", " << ptMsg_temp.point.y<< ", " << ptMsg_temp.point.z << " )"<<endl;
      std::cout<< "( " << ptMsg_temp_last.point.x << ", " << ptMsg_temp_last.point.y << ", " << ptMsg_temp_last.point.z << " )"<<endl;
  ...
  void createLookup(size_t width, size_t height)
  {
    ...
    std::cout<<"cameraMatrixColor:  "<<cameraMatrixColor<<endl;
...
int main(int argc, char **argv) 
{
  ...
  std::string topicDepth = K2_TOPIC_QHD K2_TOPIC_IMAGE_DEPTH K2_TOPIC_IMAGE_RECT;    //"hd"
