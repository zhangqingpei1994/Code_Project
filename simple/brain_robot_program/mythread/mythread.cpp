...
void forceThread::run()
{
    ...
    bool firstmove=true;
    Vector3d rc;
    rc(0)=0.0520;rc(1)=0;rc(2)=0.145;

    //Ptr<ANN_MLP> bp=ANN_MLP::load<ANN_MLP>("/home/zhang/program_hebing/brain_robot_program/bp_nn/bp_param.xml");

    PDinitial();
     Mat sample(1,6,CV_32FC1);
      Mat response;

      tcpClient2->connectToHost("192.168.1.100",30003,QTcpSocket::ReadWrite);
      qDebug()<<"tcp2连接。";
...
//    //bp_train()e ft[6];
  ...
  for(int i=0;i<6;i++)
  {
    ...
    if(ifopen&&ifstop)
      ...
      Fx=ft[0];Fy=ft[1];Fz=ft[2]+8;

      Vector3d ef;
      Vector3d et;
      ef(0)=Fx;ef(1)=Fy;ef(2)=Fz;
      et=rc.cross(ef);
...
//       v2=V6.axis()*V6.angle();
       ...
       newtarget[3]=q_actual[15]+v22(0);
       newtarget[4]=q_actual[16]+v22(1);
       newtarget[5]=q_actual[17]+v22(2);

      qDebug()<<newtarget[0]<<" "<<newtarget[1]<<" "<<newtarget[2]<<" "<<newtarget[3]<<" "<<newtarget[4]<<" "<<newtarget[5];
  if(fabs(v1(0))<1e-3&&fabs(v1(1))<1e-3&&fabs(v1(2))<1e-3&&fabs(v2(0))<1e-3&&fabs(v2(1))<1e-3&&fabs(v2(2))<1e-3){msleep(8);}
  ...
  if(erro[0]>0.031||erro[1]>0.031||erro[2]>0.036){msleep(8);}
  else{
      /*..........数据采集专用................*/
       QFile data("/home/linzc/QT/2-4/untitled_test/facexml/f_t.txt");
            if (data.open(QFile::WriteOnly | QIODevice::Append))
            {
                QTextStream out(&data);
                out <<Fx<<","<<Fy<<","<<Fz<<","<<Tx<<","<<Ty<<","<<Tz<<endl;
            }
     QFile data1("/home/linzc/QT/2-4/untitled_test/facexml/pos.txt");
