...
void detectObjectscustom(const Mat &srcImage, CascadeClassifier &cascade, vector<Rect>&faces, int DETECTION_WIDTH, int flags, Size  minFeatureSize, float searchfload, int minneighbors)
{
    Mat faceImage = srcImage;
    Mat gray;
    ...
    {
        cvtColor(faceImage, gray, CV_BGRA2GRAY);
    }
    else {
        gray = faceImage;
    }
    Mat smallImg;
    float scale = faceImage.cols / (float)DETECTION_WIDTH;
    if (faceImage.cols > DETECTION_WIDTH)
    {
        int scaledHeight = cvRound(faceImage.rows/scale);
    ...
    Mat equlizedImg;
    equalizeHist(smallImg, equlizedImg);//Ö±·œÍŒŸùÔÈ»¯£¬ÌáÉý¶Ô±È¶ÈºÍÁÁ¶È
    //Size minFeatureSize(20, 20);//×îÐ¡µÄÈËÁ³
    //float searchfload = 1.1f;//ÓÐ¶àÉÙŽóÐ¡²»Í¬µÄÈËÁ³ÒªËÑË÷
    //int minneighbors = 4;//ÉèÖÃÈËÁ³ÈçºÎ±»Œì²âµœ£¬Íš³£ÖµÎª3
    cascade.detectMultiScale(equlizedImg, faces, searchfload, minneighbors, flags, minFeatureSize);
    if (faceImage.cols > DETECTION_WIDTH)
    {
    ...
    for (int i = 0; i < (int)faces.size(); i++)
    {
        ...
        if (faces[i].y < 0)
            faces[i].y = 0;
        if (faces[i].x + faces[i].width>faceImage.cols)
            faces[i].x = faceImage.cols - faces[i].width;
        if (faces[i].y + faces[i].height>faceImage.rows)
            faces[i].y = faceImage.rows - faces[i].height;
    }
    //Rect facerect;
    //int scalewidth = 320;
...
void detectLargestObject(const Mat &img, CascadeClassifier &cascade, Rect &largestObject, int scalewidth)//scalewidth = 320
{
    int flags = CASCADE_FIND_BIGGEST_OBJECT;
    Size minfeatursize = Size(20, 20);
    float searchscalefactor = 1.1f;
    int minneighbors = 4;
    vector<Rect>objects;//¶¥µãºÍ³€¿í
    detectObjectscustom(img, cascade, objects, scalewidth, flags, minfeatursize, searchscalefactor, minneighbors);
    if (objects.size() > 0)
    {
        largestObject = (Rect)objects.at(0);//Œì²âµœµÄÈËÁ³°ŽÕÕŽóÐ¡Ë³ÐòÅÅ
    }
    else
    {
...
void detectBothEyes(const Mat &face, CascadeClassifier &eyeCascade1, CascadeClassifier &eyeCascade2, Point &leftEye, Point &rightEye, Rect *searchedLeftEye, Rect *searchedRightEye)
{
    ...
    int leftX = cvRound(face.cols * EYE_SX);//°ŽÕÕÈËÁ³Ÿ­Ñé·Ö²Œ£¬ÔÚÄ³²¿Î»Œì²âÑÛŸŠµÄŽæÔÚ
    ...
    Rect leftEyeRect, rightEyeRect;//ŽæŽ¢Œì²âµÄœá¹û
    if (searchedLeftEye)
    ...
    if (leftEyeRect.width <= 0 && !eyeCascade2.empty())//Èç¹ûÃ»Œì²âµœŸÍÔÙÀŽÒ»ŽÎ
    {
    ...
    if (leftEyeRect.width > 0) {   // Check if the eye was detected.
        ...
        leftEye = Point(-1, -1);    // Return an invalid pointÑÛŸŠµÄÖÐµã×÷Îª×óÓÒÑÛµÄÎ»ÖÃ
    }
...
if (faceRect.width > 0) {
    ...
    if (leftEye.x >= 0 && rightEye.x >= 0) {
        ...
        ellipse(mask, faceCenter, size, 0, 0, 360, Scalar(255), CV_FILLED);//žÃº¯ÊýÓÃÓÚ»­Ò»žöÍÖÔ²£¬ÍÖÔ²µÄÖÐÐÄÊÇÏÞ¶šŸØÐÎµÄÖÐÐÄ£¬Ê¹ÓÃµ±Ç°»­±Ê»­ÍÖÔ²£¬ÓÃµ±Ç°µÄ»­Ë¢Ìî³äÍÖÔ²¡£
        //imshow("mask", mask);
