...
void learnCollectedFaces( const Ptr<BasicFaceRecognizer> &model,const vector<Mat> preprocessedFaces, const vector<int> faceLabels, const string facerecAlgorithm)
{
...
Mat reconstructFace(const Ptr<BasicFaceRecognizer> &model, const Mat &preprocessedFace)
{
    ...
    try {
        ...
        Mat eigenvectors = model->getEigenVectors();//»ñÈ¡ÌØÕ÷ÏòÁ¿
        Mat averageFaceRow = model->getMean();//»ñÈ¡ÆœŸùÈËÁ³
