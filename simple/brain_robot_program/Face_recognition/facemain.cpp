...
const char *faceCascadeFilename = "/home/linzc/QT-workspace/untitled_test_1/facexml/haarcascade_frontalface_alt2.xml";
const char *eyeCascadeFilename1 = "/home/linzc/QT-workspace/untitled_test_1/facexml/haarcascade_lefteye_2splits.xml";   // Best eye detector for open-or-closed eyes.
const char *eyeCascadeFilename2 = "/home/linzc/QT-workspace/untitled_test_1/facexml/haarcascade_righteye_2splits.xml";   // Best eye detector for open-or-closed eyes.
...
void onMouse(int event, int x, int y, int, void*)
{
    ...
    {//Œì²éµã»÷µÄÊÇ·ñŽŠÓÚÌíŒÓÈËÎï°ŽÅ¥
        ...
        if ((m_numPersons == 0) || (numbersele >=10))
        {
            ...
            string filename;
            filename ="/home/linzc/QT/2-4/untitled_test/facexml/"+stream1.str();
            status = 0;
            status = mkdir(filename.c_str(),S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
            m_numPersons++;
            status = m_numPersons + initial;
            outfile.open("/facexml/myfile.dat", std::ofstream::binary);
...
int onlyrecognized( Ptr<BasicFaceRecognizer> &model,VideoCapture &videoCapture, CascadeClassifier &faceCascade, CascadeClassifier &eyeCascade1, CascadeClassifier &eyeCascade2)
{
    string outputStr;
    double similarity;
...
void recognizeAndTrainUsingWebcam( const Ptr<BasicFaceRecognizer> &model,VideoCapture &videoCapture, CascadeClassifier &faceCascade, CascadeClassifier &eyeCascade1, CascadeClassifier &eyeCascade2)
{
        ...
        else if (m_mode == MODE_COLLECT_FACES)
        {
            ...
            {//Œì²âµœÈËÁ³Ê±²ÅÖŽÐÐ
                ...
                {// CHANGE_IN_IMAGE_FOR_COLLECTION=0.3ãÐÖµ  CHANGE_IN_SECONDS_FOR_COLLECTION=0.1Ò»Ãë²ÉŒ¯Ò»ÕÅ
                    ...
                    string filename;
                    filename = "/home/linzc/QT/2-4/untitled_test/facexml/" + stream1.str() + "/" + stream.str()+".jpg";
            ...
            if (haveEnoughData) {
                ...
                m_mode = MODE_RECOGNITION;
                model->save("/home/linzc/QT/2-4/untitled_test/facexml/Fisher_Model.xml");
            }
            else {
...
int recognized()
{
    ...
    ifstream inF;
    inF.open("/home/linzc/QT-workspace/untitled_test_1/facexml/myfile.dat", std::ifstream::binary);
    inF.read(reinterpret_cast<char*>(&initial), sizeof(int));
    inF.close();
    model->load("/home/linzc/QT-workspace/untitled_test_1/facexml/Fisher_Model.xml");
...
void facebegin()
{
    ...
    ifstream inF;
    inF.open("/home/linzc/QT/2-4/untitled_test/facexml/myfile.dat", std::ifstream::binary);
    inF.read(reinterpret_cast<char*>(&initial), sizeof(int));
    inF.close();
    model->load("/home/linzc/QT/2-4/untitled_test/facexml/Fisher_Model.xml");
    //initial = m_numPersons;
    //waitKey(5000);
    cout << "WebcamFaceRec, by Shervin Emami (www.shervinemami.info), June 2012." << endl;
    cout << "Realtime face detection + face recognition from a webcam using LBP and Eigenfaces or Fisherfaces." << endl;
