...
void Robot_arm::track_head(Eigen::Vector4d & nose1,Eigen::Vector4d &nose2,double speed)
{
  if(speed<0.005)
  {
     target_nose1=handeye_data_Affin3d*nose1+differ1;
     target_nose2=handeye_data_Affin3d*nose2+differ2;

     Eigen::Vector4d target=(target_nose1+target_nose2)/2;
     std::cout<<"target: "<<target(0,0)<<" "<<target(1,0)<<" "<<target(2,0)<<std::endl;
     movelpos(target(0,0), target(1,0), target(2,0), Rx, Ry, Rz, 0.1, 0.1);

  }

}

void Robot_arm::get_position(double t_x,double t_y,double t_z)
{
       arm_X=t_x;
       arm_Y=t_y;
       arm_Z=t_z;
...
/*void Robot_arm::Write_data_to_txt(double data1,double data2,double data3,double data4,double data5,double data6)
{
