...
#define pi 3.1415926;
forceThread::forceThread(QObject *parent) : QThread(parent)
{
    moveToThread(this);
}
    ...
    while (1)
    {
        if (ifok)
        {
            ...
            if (ifopen && ifstop)
            {
                ...
                ft[0] = (*forcevalue) / 10 - 1.7;
                ft[1] = (*(forcevalue + 1)) / 10 + 0.5;
                ft[2] = (*(forcevalue + 2)) / 10 + 12.3;
                ft[3] = (*(forcevalue + 3)) / 1000 - 0.033;
                ft[4] = (*(forcevalue + 4)) / 1000 - 0.239;
                ft[5] = (*(forcevalue + 5)) / 1000 - 0.037;
                ...
                double bp[6];
                bp[0] = 20 * response.at<float>(0, 0);
                bp[1] = response.at<float>(0, 1) * 20;
                bp[2] = response.at<float>(0, 2) * 20;
                bp[3] = response.at<float>(0, 3) * 1.2;
                bp[4] = response.at<float>(0, 4) * 1.2;
                bp[5] = response.at<float>(0, 5) * 1.2;
                Fx = ft[0] - bp[0];
                Fy = ft[1] - bp[1];
                Fz = ft[2] - bp[2] + 8;
                ...
                Tx = ft[3] - bp[3] + Fy * 0.045;
                Ty = ft[4] - bp[4] + (Fz - 5) * 0.154 - Fx * 0.045;
                Tz = ft[5] - bp[5] - Fy * 0.154;
                // else
