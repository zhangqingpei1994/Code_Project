...
namespace kfusion
{
    ...
    public:        
        ...
        const KinFuParams& params() const;   //返回值是一个引用
        KinFuParams& params();
        ...
        bool operator()(const cuda::Depth& dpeth, const cuda::Image& image = cuda::Image());  //仿函数
    ...
    private:
        ...
        cuda::Frame curr_, prev_;   // Frame 里面包含了三个vector,类型分别为Cloud,Normals,Depth

        cuda::Dists dists_;
        cuda::Cloud points_;
        cuda::Normals normals_;
        cuda::Depth depths_;        //cuda::DeviceArray2D<unsigned short>
